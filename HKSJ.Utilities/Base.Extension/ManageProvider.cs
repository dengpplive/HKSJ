...
namespace HKSJ.Utilities
{
    ...
    public class ManageProvider : IManageProvider
    {
        ...
        public T Current<T>()
        {
            var user = JsonConvert.DeserializeObject<T>(LoginProvider == "Cookie" ? DESEncrypt.Decrypt(CookieHelper.GetCookie(LoginUserKey)) 
                : DESEncrypt.Decrypt(SessionHelper.Get(LoginUserKey).ToString()));
            return user;
        }
    }
}
