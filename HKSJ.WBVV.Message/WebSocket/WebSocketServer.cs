...
namespace HKSJ.WBVV.Message.WebSocket
{
    public class Logger
    {
    ...
    public class WebSocketServer : IDisposable
    {
        ...
        public WebSocketServer()
        {
            ...
            Initialize();
            //ServerPort = 8800;
            //ServerLocation = "ws://localhost:8800/api/UserMessage/Comment";
            //Initialize();
        }
        ...
        void socketConn_BroadcastMessage(Object sender, string message, EventArgs e)
        {
            ...
            {
                var obj = message.FromJSON<Newtonsoft.Json.Linq.JObject>();
                var messageId = obj.Value<int>("messageId");
                string url = messageId <= 0 ? "UserMessage/Comment" : "UserMessage/Reply";
                var result = WebApiHelper.InvokeApi<Result>(url, obj);
                if (result.Success)
                {
                    var usermessagedetialsview = WebApiHelper.InvokeApi<UserMessageDetialsView>("UserMessage/GetComment?messageId={0}".F(result.Data));
                    var newObj =new 
                    {
                        messageId=messageId,
                        usermessagedetialsview=usermessagedetialsview
                    };
                    Send(newObj.ToJSON());
                }
            }
